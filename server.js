...
const liveSockets = new Set(); // every currently-open ws TCP socket, for periodic keepalive pings

// ---- minimal WebSocket framing ----
function encodeFrame(data, opcode) {
  opcode = opcode === undefined ? 0x1 : opcode; // 0x1 text, 0x9 ping, 0xA pong
...
function decodeFrames(buf, onMessage) {
  ...
  while (off + 2 <= buf.length) {
    ...
    else if (opcode === 9) onMessage({ ping: data }); // ping -> caller replies pong
    // opcode 10 (pong) needs no action
    off = p + len;
  }
...
server.on('upgrade', (req, socket) => {
  ...
  console.log('[upgrade]', req.url, 'match=', !!m, 'key=', !!key, 'from', req.socket.remoteAddress);
  if (!m || !key || (req.headers.upgrade || '').toLowerCase() !== 'websocket') { console.log('[upgrade] rejected'); socket.destroy(); return; }
  ...
  socket.on('data', (chunk) => {
    try {
      ...
      buf = decodeFrames(buf, (msg) => {
        if (msg === null) socket.end();
        else if (msg && msg.ping) { try { socket.write(encodeFrame(msg.ping, 0xA)); } catch (e) {} }
        else onMessage(socket, m[1].toUpperCase(), msg);
      });
    } catch (e) { console.error('frame error:', e); try { socket.destroy(); } catch (e2) {} }
  });
...
function onMessage(socket, code, raw) {
  ...
  try { onMessageInner(socket, code, m); } catch (e) { console.error('handler error:', e); }
}
function onMessageInner(socket, code, m) {
  console.log('[msg]', code, m.t, 'pid=', m.pid, 'roomExists=', rooms.has(code));
...
server.on('clientError', (err, socket) => console.log('[clientError]', err.message));
// mobile/carrier NAT commonly drops idle connections within ~30-60s; ping every 25s to keep them alive
setInterval(() => {
  for (const s of liveSockets) { try { s.write(encodeFrame(Buffer.alloc(0), 0x9)); } catch (e) { liveSockets.delete(s); } }
}, 8000); // Render's proxy appears to drop idle connections within ~15-20s, so ping well inside that
